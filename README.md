# Walker
## Project Overview 🔍
I'm creating a 2-legged walking robot to learn how inverse kinematics works, robot design, and because it seemed like an awesome idea.
## Goals 🥅
-Build a stable robot

-Explore inverse kinematics and trigonometry

-Create walking sequence 

-Iterate on designs until robot is able to walk smoothly

## Repository files 📁
- /designs: Designs for the robot, including sketches and CAD
- /code: Code for the robot
- /media: Photos and videos of tests
- /docs: Notes and calculations

## Development log 🗒️

### Design
### **----Design: Iteration1----**

<i>Iteration 1 on the robot design</i>

➡️ [View Iteration 1](https://github.com/ArifNaufalMNazri/Walker/blob/main/designs/iteration1/README.md)

**Synopsis**: I sketched a few robot designs in my notebook, and settled on the "Ostrich" sketch



### **----Design: Iteration2----**

<i>Iteration 2 on the robot design</i>

➡️[View Iteration 2](https://github.com/ArifNaufalMNazri/Walker/blob/main/designs/iteration2/README.md)

**Synopsis**: I made the 3d CAD design of my design sketch


### Prototype Leg
### **----Prototype Leg: Iteration 1----**

<i>Iteration 1 on the prototype leg</i>

➡️[View Iteration 1](https://github.com/ArifNaufalMNazri/Walker/blob/main/protoLeg/iteration1/README.md)

**Synopsis**: I built the first protoype leg

### **----Prototype Leg: Iteration 2----**

<i>Iteration 2 on the prototype leg</i>

➡️[View Iteration 2](https://github.com/ArifNaufalMNazri/Walker/blob/main/protoLeg/iteration2/README.md)

**Synopsis**: I conducted the button test with the prototype leg

### **----Prototype Leg: Iteration 3----**

<i>Iteration 3 on the prototype leg</i>

➡️[View Iteration 3](https://github.com/ArifNaufalMNazri/Walker/blob/main/protoLeg/iteration3/README.md)

**Synopsis**: I calculated and programmed the kinematics and built a new stand for leg testing

### **----Prototype Leg: Iteration 4----**

<i>Iteration 4 on the prototype leg</i>

➡️[View Iteration 4](https://github.com/ArifNaufalMNazri/Walker/blob/main/protoLeg/iteration4/README.md)

**Synopsis**: I created a new stand for the leg and replaced the hip servo

### **----Prototype Leg: Iteration 5----**

<i>Iteration 5 on the prototype leg</i>

➡️[View Iteration 5](https://github.com/ArifNaufalMNazri/Walker/blob/main/protoLeg/iteration5/README.md)

**Synopsis**: I changed the code to allow the leg to move in the -x axis, added supports to the stand, and created a triangle stepping sequence
